...
def render_audit_report(
    ...
    *,
    ...
    verdict = analysis.get("verdict", "(verdetto non specificato)")
    ...
    if minority:
        body_parts.append(f"""
        <h2>⚠️ Minority Report (caso CONTRO)</h2>
        <div class="card crit">{e(minority)}</div>
        """)

    # Open Questions
    if open_qs:
        body_parts.append("<h2>❓ Open Questions</h2><ul>")
        for q in open_qs:
    ...
    # Opinioni per agente
    if agent_opinions:
        body_parts.append("<h2>👥 Per Agente</h2>")
    ...
    # Footer
    body_parts.append("""
    <footer>Generato da tavolarotonda v0.1.0 — Council of High Intelligence pattern
    (18 personas, polarity pairs, adversarial evidence, anti-groupthink).</footer>
    """)

    return HTML_BASE.format(title=e(title), body="\n".join(body_parts))
...
def render_qa_template(
    ...
    *,
    ...
    body.append('<div class="meta">Template Q&A multi-domanda. '
                'Compila le domande, il council risponde in parallelo e genera analisi comparativa.</div>')

    # Form per aggiungere/modificare domande
    body.append('<h2>✏️ Domande</h2>')
    ...
    <button onclick="runAnalysis()" style="margin-top:8px;padding:10px 16px;background:var(--accent);
      ...
      ▶️ ESEGUI ANALISI MULTI-AGENTE
    </button>
    """)

    # Area risultati
    body.append('<h2>📊 Risultati</h2>')
    body.append('<div id="results">'
                '<p style="color:var(--muted)">Premi "ESEGUI ANALISI" per popolare questa sezione.</p>'
                '</div>')

    # Se abbiamo risposte precompilate, mostriamo l'analisi comparativa
    if prefill_answers:
        body.append("<hr><h2>🔍 Analisi Comparativa</h2>")
        for i, q in enumerate(questions):
    ...
    body.append("<footer>Template Q&A generato da tavolarotonda.</footer>")
    return HTML_BASE.format(title=e(title), body="\n".join(body))


# === AUDIT REPORT DAL PALACE (auto-generato) ===

def audit_report_from_palace(palace: MemoryPalace, target_name: str, target_description: str) -> str:
    ...
    analysis = {
        ...
        "verdict": palace.decision or "(verdetto non ancora prodotto)",
        "confidence": palace.convergence_score,
        "minority_report": palace.minority_report or "",
        "open_questions": palace.open_questions,
    }
    return render_audit_report(target_name, target_description, analysis)
